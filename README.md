...
```
`wait_ms`はそのstepを呼ぶ直前の待機です。`tool`は内部名の`resume`/`call_pscript_mcp`だけでなく、AIから実際に見える`desmume_harness__resume`/`desmume_harness__call_pscript_mcp`形式も受け付けます。実行対象の既存laneでは、最初のstep前にページAPI `setUiInteractionLock` を実行ごとに一意なowner付きで有効化し、macro終了時に`finally`でその実行ownerだけ解除します。同じmacro IDの並列実行も互いのロックを解除しません。これは人間のGUI操作だけを止めるため、macro自身の`pause`、永続スクリプト由来のpause、`DesmumeMCP.call()`などのプログラム操作は妨げません。解決時はharnessの`TOOLS`一覧を総当たりし、完全一致を優先したうえで`__<tool name>`の後方一致を一意に解決して内部名へ正規化します。`micro_macro_exec`側へroot `isolation_id`を指定すると、`isolation_id`を受け付ける全stepへその値を継承します。ただしstep自身の`arguments.isolation_id`が明示されている場合はAI側の指定を優先し、root値で上書きしません。`isolation_id`非対応toolへはroot値を注入しません。初回は`steps`付きで登録・実行し、以後は`{ "id": "battle-turn-flow", "isolation_id": "lane-a" }`だけで同じ手順を再実行できます。`micro_macro_list`と`micro_macro_get`は実行せず保存内容だけを確認します。マイクロマクロから別の`micro_macro_*`を呼ぶ再帰実行は行いません。
## start_analyze
